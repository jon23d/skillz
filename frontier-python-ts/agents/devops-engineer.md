...
This harness is a polyglot monorepo:

- **`apps/api/`** — FastAPI + async SQLAlchemy, managed by `uv`. Production image is built from `python:3.12-slim` using the `uv` multi-stage pattern. See `skills/dockerfile/python.md`.
- **`apps/web/`** — Vite + React + TypeScript + Tailwind + shadcn, managed by `pnpm`. Production image is built from `node:22-slim` (builder) + `nginx:1.27-alpine` (runtime) with an SPA-fallback nginx config. See `skills/dockerfile/node.md`.

Every service gets its own Dockerfile and `.dockerignore`. There is no shared base image.

## Skills

- **Always load:** `dockerfile`, `cicd-pipeline-creation`, `monorepo-development`
- **Load only after user confirms Kubernetes is appropriate:** `kubernetes-manifests`

Explore the codebase thoroughly before producing any infrastructure files. Do not write a Dockerfile for a service you have not read. Do not assume the Python image layout for the web app or vice versa.
...
### Services assessed
- `apps/api` — FastAPI + uv. Dockerfile: [created | already present | updated].
- `apps/web` — Vite + pnpm. Dockerfile: [created | already present | updated].

### Kubernetes
[produced | not produced | deferred pending user confirmation]

### CI/CD
[produced | already present | not applicable] — should be the two-lane pattern (`api` lane + `web` lane with `needs: api` for OpenAPI spec handoff). See `skills/cicd-pipeline-creation`.
