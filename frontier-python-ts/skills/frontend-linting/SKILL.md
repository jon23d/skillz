...
description: Use when setting up ESLint and/or Prettier in the Vite + React + TypeScript frontend, adding linting to an existing frontend codebase, or configuring typescript-eslint, eslint-config-prettier, or related packages. Backend (Python) linting lives in the python-linting skill.
---

# Frontend Linting Setup (Vite + React + TS)

## Overview

ESLint v9+ with flat config (`eslint.config.ts`) and Prettier as a separate formatter. The Vite + React + TypeScript frontend uses type-aware rules. Python backend linting is governed by the `python-linting` skill.
