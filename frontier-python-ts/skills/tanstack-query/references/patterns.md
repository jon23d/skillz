...
function UserPage({ userId }: { userId: string }) {
  return (
    <Suspense fallback={<Skeleton className="h-[200px] w-full" />}>
      <ErrorBoundary fallback={<div className="text-destructive">Failed to load user</div>}>
...
// Tiny local debounce hook — no dependency needed.
...
function AutocompleteUsers() {
  ...
  const { data: suggestions } = useQuery({
    ...
    queryFn: () => autocompleteUsers(debouncedSearch),
  ...
  return (
    <Command>
      <CommandInput value={searchInput} onValueChange={setSearchInput} />
      <CommandList>
        {suggestions?.map((u) => (
          <CommandItem key={u.id} value={u.id}>{u.name}</CommandItem>
        ))}
      </CommandList>
    </Command>
  );
...
`<Command>`, `<CommandInput>`, `<CommandList>`, `<CommandItem>` come from shadcn's `command` component (built on `cmdk`).
